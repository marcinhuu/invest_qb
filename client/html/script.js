...
$('form .btn').click(function (e) {
    ...
    if(!inputValue || /^\d+$/.test(inputValue.value)) {
        ...
        if (activeMenu == "sell") {
            $.post('http://invest_qb/sellInvestment', JSON.stringify({job: label}))
        } else if(activeMenu == "buy") {
            $.post('http://invest_qb/buyInvestment', JSON.stringify({job: label, amount: inputValue, boughtRate: rate}))
        }
...
$('#new_bank').click(function() {
    $.post('http://invest_qb/newBanking', "{}")
})
...
function mainPage() {
    ...
    $('#general').show();
    inMenu = false
    $.post('http://invest_qb/balance', "{}")
}

function close() {
    $('#general, #waiting, #sellUI, #allUI, #buyUI, #topbar').hide();
    $('body').removeClass("active");
    $.post('http://invest_qb/close', "{}");
}